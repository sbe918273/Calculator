...
/**
 * A class to represent a lexer that reads an input file to generate tokens with attributes.
...
public class Lexer {
        ...
        while (peek != null && Character.isDigit(peek)) {
            ...
            if (isZero) {
                throw new LeadingZeroException(String.format(
                    "[Lexer:getInteger %d:%d] Illegal leading zero.",
                    lineNumber,
                    characterNumber
                ));
            }
    ...
    protected Integer getOptionalSignedInteger() throws IOException, LeadingZeroException, MissingIntegerException {
        ...
        if (integer == null) {
            ...
            if (sign != null) {
                ...
                "[Lexer:getOptionalSignedInteger %d:%d] Sign without a following integer.",
                lineNumber,
                characterNumber
            ));
            }
     ...
     */
    protected Token getOptionalNumberToken() throws
            IOException,
            ...
            LeadingZeroException
    {
        ...
        if (integer == null) {
            if (fractional && fraction == null) {
                ...
                throw new EmptyNumberException(String.format(
                    "[Lexer:getNumberToken %d:%d] Illegal empty number.",
                    lineNumber,
                    characterNumber
                ));
            } else if (!fractional) {
        ...
        if (peek != null && peek == 'e') {
            ...
            if (exponent == null) {
                throw new MissingIntegerException(String.format(
                    "[Lexer:getNumberToken %d:%d] Missing exponent after 'e'.",
                    lineNumber,
                    characterNumber
                ));
            }
    ...
    protected TagToken getOptionalCosineToken() throws IOException, IncompleteCosineException {
        ...
        if (!(readCharacter('o') && readCharacter('s'))) {
            throw new IncompleteCosineException(String.format(
                "[Lexer:getOptionalCosineToken %d:%d] 'c' should be a prefix to \"cos\".",
                lineNumber,
                characterNumber
            ));
        }
    ...
    public Token scan() throws IOException, InvalidTokenException {
        ...
        if (token == null) {
            throw new IllegalCharacterException(String.format(
                "[Lexer:scan %d:%d] Illegal character",
                lineNumber,
                characterNumber
        )); }
     ...
     * @throws InvalidTokenException the current input produces an invalid token
     */
    public Token[] completeScan() throws IOException, InvalidTokenException {
